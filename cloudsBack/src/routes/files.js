...
// Every file operation is scoped to the signed-in user (req.userId).
async function prepare(userId, providerName) {
  ...
  const token = await getValidAccessToken(userId, providerName);
  return { adapter, token };
}

router.get("/:provider/files", auth, async (req, res) => {
  try {
    const { adapter, token } = await prepare(req.userId, req.params.provider);
...
router.get("/:provider/files/:id/view", auth, async (req, res) => {
  try {
    const { adapter, token } = await prepare(req.userId, req.params.provider);
...
router.post("/:provider/files", auth, upload.single("file"), async (req, res) => {
  try {
    const { adapter, token } = await prepare(req.userId, req.params.provider);
    if (!req.file) {
...
router.patch("/:provider/files/:id", auth, async (req, res) => {
  try {
    const { adapter, token } = await prepare(req.userId, req.params.provider);
...
router.delete("/:provider/files/:id", auth, async (req, res) => {
  try {
    const { adapter, token } = await prepare(req.userId, req.params.provider);
